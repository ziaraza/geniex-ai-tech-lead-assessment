...
**Alternative:** Use tool/function calling or structured JSON output (`response_format`) so the model returns machine-readable fields with zero regex, eliminating the entire parsing error surface.
...
**Verdict:** Disagree
**Why:** After the tool loop exits on `stop_reason == "end_turn"`, `content[0]` is assumed to be a text block, but the SDK can return other block types (e.g. a trailing `tool_use` block), causing an `AttributeError` that crashes the pipeline.
